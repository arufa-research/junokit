...
//         ),
//           `Expected ${accountAddresses.toString()} to
// change balance by ${balanceChanges.toString()} ${token}, ` +
//             `but it has changed by ${actualChanges.toString()} ${token}`,
//           `Expected ${accountAddresses.toString()} to not change balance
// by ${balanceChanges.toString()} ${token},`,
...
//   token: string
// ): Promise<number[]> {
//   const client = getClient(JunokitContext.getJunokitContext().getRuntimeEnv().network);
