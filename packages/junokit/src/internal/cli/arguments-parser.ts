...
export class ArgumentsParser {
  ...
  public static cLAToParamName (cLA: string): string {
    if (cLA.toLowerCase() !== cLA) {
      throw new JunokitError(ERRORS.ARGUMENTS.PARAM_NAME_INVALID_CASING, {
        param: cLA
      });
    }
  ...
  // eslint-disable-next-line
    ...
    for (let i = 0; i < rawCLAs.length; i++) {
      ...
      if (!this._isCLAParamName(arg, taskDefinition.paramDefinitions)) {
        throw new JunokitError(ERRORS.ARGUMENTS.UNRECOGNIZED_PARAM_NAME, {
          param: arg
        });
      }
  ...
  _parseArgumentAt (
    ...
    if (parsedArguments[paramName] !== undefined) {
      throw new JunokitError(ERRORS.ARGUMENTS.REPEATED_PARAM, {
        param: claArg
      });
    }
  ...
  _parsePositionalParamArgs (
    ...
    for (let i = 0; i < positionalParamDefinitions.length; i++) {
      ...
      if (rawArg === undefined) {
        if (!definition.isOptional) {
          throw new JunokitError(ERRORS.ARGUMENTS.MISSING_POSITIONAL_ARG, {
    ...
    if (
      ...
      throw new JunokitError(ERRORS.ARGUMENTS.UNRECOGNIZED_POSITIONAL_ARG, {
